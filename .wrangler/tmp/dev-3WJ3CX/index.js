...
__name(json, "json");
function error3(msg, status = 400) {
...
var worker_default = {
  async fetch(request, env2) {
    ...
    if (path === "/api/health" && method === "GET") {
      return json({ ok: true, timestamp: (/* @__PURE__ */ new Date()).toISOString() });
    }
    if (path === "/api/deposit/config" && method === "GET") {
      const wallet = await env2.KV.get("config:wallet_address") || env2.WALLET_ADDRESS;
      const network = await env2.KV.get("config:network") || env2.WALLET_NETWORK;
      const telegram = await env2.KV.get("config:telegram") || env2.TELEGRAM;
    ...
    if (playerMatch && method === "GET") {
      ...
      if (!name) return error3("Player name required");
    ...
    if (path === "/api/admin/deposits" && method === "POST") {
      ...
      if (!playerName || !amount) {
        return error3("Jugador y cantidad son requeridos");
      }
      const numAmount = parseFloat(amount);
      if (isNaN(numAmount) || numAmount <= 0) {
        return error3("Cantidad inv\xE1lida");
      }
      ...
      const existing = await env2.DB.prepare("SELECT id FROM deposits WHERE tx_hash = ?1").bind(txHashClean).first();
      if (existing) {
        return error3("Esta transacci\xF3n ya fue registrada.", 409);
      }
      ...
      const deposit = await env2.DB.prepare("SELECT * FROM deposits WHERE tx_hash = ?1").bind(txHashClean).first();
      return json({
        ok: true,
        deposit: {
          id: deposit?.id,
    ...
    if (adminPlayerMatch && method === "GET") {
      ...
      return json({
        totalDeposited: result?.total_deposited || 0,
        depositCount: result?.deposit_count || 0
      });
    }
    return error3("Not found", 404);
  }
};
...
// .wrangler/tmp/bundle-VzIEUt/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-VzIEUt/middleware-loader.entry.ts
